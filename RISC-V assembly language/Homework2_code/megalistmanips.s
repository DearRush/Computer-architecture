...
mapLoop:
    lw t1 0(s0) # load the address of the array of current node into t1 把add改成lw
    lw t2, 4(s0)        # load the size of the node's array into t2 

    addi t3,x0,4        #修改，每次调整地址应该考虑int的大小
mul t3 t0 t3
add t1, t1,t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0
    
addi sp, sp, -4      #修改，t1值需要保存
sw t1 0(sp)
...
addi sp sp 4
    ...
    lw a0, 8(s0)   # load the address of the next node into a0,修改，使用lw指令而不是la
    add a1 x0 s1   # put the address of the function back into a1 to prepare for the recursion 修改，用add指令而不是ld
